...
- Embed `liskov-runtime-contact` v0.2.8 through an exact five-path overlay,
  ...
  remainder of its shared 15-second budget. A first `processor_version` timeout
  receives one bounded retry with a fresh ID to distinguish bridge-dispatch
  startup races from a persistently unresponsive bridge; the incompatible
  long-ID comparison remains bounded evidence only.
